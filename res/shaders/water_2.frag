...
layout (std140, binding = 6) uniform DistanceFieldData
{
    mat4 dFProjection;
    mat4 dFView;
	vec3 dFPos;
...
void main() {
    ...
    result += vec4(vec3(texture(distanceFieldMap, vec2(fs_in.texCoord.x, map(fs_in.texCoord.y, 0.0, 1.0, 1.0, 0.0))).r + 0.1), 1.0);
    ...
    //Color = vec4(vec3(pow(texture(distanceFieldMap, fs_in.texCoord).rgb + 0.1, vec3(gamma))), 1.0);
    //Color = vec4(vec3(pow(DistanceFieldCalculation().rgb, vec3(gamma))), 1.0);
}