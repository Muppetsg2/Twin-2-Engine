...
std::vector<float> generate3DPerlinNoise(int width, int height, int depth, float scale) {
    std::vector<float> noiseData(width * height * depth);

    for (int z = 0; z < depth; ++z) {
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                float fx = x / (float)width * scale;
                float fy = y / (float)height * scale;
                float fz = z / (float)depth * scale;
                noiseData[x + y * width + z * width * height] = stb_perlin_noise3(fx, fy, fz, 0, 0, 0);
                //noiseData[x + y * width + z * width * height] = stb_perlin_noise3_wrap_nonpow2(fx, fy, fz, 0, 0, 0);
            }
        }
    }
...
const int numFeaturePoints = 8;  // Number of feature points per grid cell
    ...
    for (int dz = -1; dz <= 1; ++dz) {
        for (int dy = -1; dy <= 1; ++dy) {
            for (int dx = -1; dx <= 1; ++dx) {
                ...
                std::default_random_engine generator(nx + ny * 57 + nz * 131);
                std::uniform_real_distribution<float> distribution(0.0f, 1.0f);

                for (int i = 0; i < numFeaturePoints; ++i) {
                    float px = nx + distribution(generator);
                    float py = ny + distribution(generator);
                    float pz = nz + distribution(generator);
...
std::vector<float> generate3DWorleyNoise(int width, int height, int depth, float scale) {
    std::vector<float> noiseData(width * height * depth);
    ...
    for (int z = 0; z < depth; ++z) {
        for (int y = 0; y < height; ++y) {
            for (int x = 0; x < width; ++x) {
                ...
                float fy = y / (float)height * scale;
                float fz = z / (float)depth * scale;
                noiseData[x + y * width + z * width * height] = worleyNoise(fx, fy, fz);
            }
        }
    }
...
std::vector<float> generate3DPerlinWorleyNoise(int width, int height, int depth, float scale) {
    auto perlinNoise = generate3DPerlinNoise(width, height, depth, scale);
    auto worleyNoise = generate3DWorleyNoise(width, height, depth, scale);

    std::vector<float> combinedNoise(width * height * depth);

    for (size_t i = 0; i < combinedNoise.size(); ++i) {
        // Combine Perlin and Worley noise (you can adjust this formula as needed)
        combinedNoise[i] = 0.5f * perlinNoise[i] + 0.5f * worleyNoise[i];
    }
