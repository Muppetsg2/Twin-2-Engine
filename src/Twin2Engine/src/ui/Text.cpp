...
void Text::SetText(const string& text)
{
	if (_text != text) {
		...
		// Dodawanie kolejnych
		if (_textCache.size() < text.size()) {
			vector<Character*> chars = _font->GetText(text.substr(_text.size() - 1, text.size() - _text.size() - 1), _size);
			for (auto& c : chars) _textCache.push_back(c);
		}
		// Odejmowanie nadmiaru
		else if (_textCache.size() > text.size()) {
